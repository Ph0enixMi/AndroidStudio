...
public class MainActivity extends AppCompatActivity {
    ...
    public void onTimeDialog(int hour, int minute){
        Toast.makeText(getApplicationContext(), "Вы выбрали ".concat(Integer.toString(hour)).concat(" часов ").concat(Integer.toString(minute)).concat(" минут"), Toast.LENGTH_LONG).show();
    }

    public void onDateDialog(int year, int month, int day){
        Toast.makeText(getApplicationContext(), "Вы выбрали ".concat(Integer.toString(day)).concat(".").concat(Integer.toString(month)).concat(".").concat(Integer.toString(year)), Toast.LENGTH_LONG).show();
    }
}